...
{
    public partial class pListbyINV : Form
    {
        public pListbyINV()
        {
            InitializeComponent();
        ...
        private void sheetcombine(int start, int end)
        {
            ...
            try
            {
                ...
                genBook.SaveAs(rootDir + @"\INV\" + "INV" + start + "-" + end + ".xlsx");
                genBook.Dispose();
                GC.Collect();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    if (saveFileDialog.ShowDialog() != DialogResult.Cancel)
                    {
                        try
                        {
                            string remote = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + @"\data\INV";
                            string local = saveFileDialog.FileName;
                            string sourceDir = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + @"\data\INV";
